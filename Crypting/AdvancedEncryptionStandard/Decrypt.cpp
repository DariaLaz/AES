...
* @compiler VC
*
* <file with functions related with the decrypting algorithm>
*
*/
...
void DecryptRijndaelKeyShedule(int key[4][4]) {
    for (size_t row = 0; row < 4; row++)
    {
        ...
        {
            InverseSBox(key[row][col]);
        }
        ReverseRotate(key[row]);
    }
}

void ReverseKeyExtension(int key[4][4], int round = -1) {
    int rcons[10] = { 54, 27, 128, 64, 32, 16, 8, 4, 2, 1 };
...
void ReversedShiftRows(int text[4][4]) {
    for (size_t row = 0; row < 4; row++)
    {
        ReversedShiftElements(text[row], row);
    }
}
